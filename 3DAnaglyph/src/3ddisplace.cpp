...
{
	// переменные
	_awpColor* css = NULL; // исходное изображение
    _awpColor* cdd = NULL; // полученное изображение
    ...
    css = (_awpColor*)src->imageData;
    cdd = (_awpColor*)dst->imageData;
    d = (unsigned char*)map->imageData;
	for (y = 0; y < src->height; y++)
    {
        yy = y;
		cdd = (_awpColor*)((unsigned char*)dst->imageData + y*dst->widthStep);
		css = (_awpColor*)((unsigned char*)src->imageData + y*src->widthStep);
		d = (unsigned char*)map->imageData + y*map->widthStep;
		if (delta == -1)
		{
